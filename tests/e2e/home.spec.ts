...
test("homepage loads, processes a seeded chamber, previews, and downloads a PDF", async ({
  page
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByText(/small business breakfast briefing/i)
  ).toBeVisible();
...
test("shows graceful feedback when no events are returned", async ({
  page
}) => {
      ...
      body: JSON.stringify({
        ...{
          ...
          viewModel: {
            ...
            footerAttribution:
              "Source: Columbus Chamber public events calendar",
            emptyStateMessage:
              "No upcoming public events were found on the selected chamber page.",
            branding: {
              logoUrl: null,
              faviconUrl: null,
              themeColor: "#113456",
  ...
  await expect(
    page.getByText(/no upcoming public events found/i)
  ).toBeVisible();
});

test("shows a friendly inline error when preview generation fails", async ({
  page
}) => {
  await page.route("**/api/newsletter/preview", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        error: {
          code: "FETCH_FAILED",
          message:
            "The chamber events page could not be reached. Please try again shortly."
        }
      })
    });
  });
