...
describe("detectPlatform", () => {
  it("prefers an explicit platform hint", async () => {
    ...
    expect(
      detectPlatform(seededChambers[0], html, seededChambers[0].eventsUrl)
    ).toBe("jsonld-event-adapter");
  });
  ...
  it("detects json-ld event pages", async () => {
    ...
    expect(
      detectPlatform(
        { ...seededChambers[0], platformHint: null },
        html,
  ...
  it("detects growthzone pages", async () => {
    ...
    expect(
      detectPlatform(
        { ...seededChambers[1], platformHint: null },
        html,
        seededChambers[1].eventsUrl
      )
    ).toBe("growthzone-family-adapter");
  });
  ...
  it("detects wordpress events pages", async () => {
    ...
    expect(
      detectPlatform(
        { ...seededChambers[4], platformHint: null },
        html,
        seededChambers[4].eventsUrl
      )
    ).toBe("wordpress-events-adapter");
  });

  it("falls back to the generic adapter when no platform signal is present", async () => {
    const html = await readChamberFixture("generic-westerville.html");
    expect(
      detectPlatform(
        { ...seededChambers[3], platformHint: null },
        html,
        seededChambers[3].eventsUrl
      )
    ).toBe("generic-calendar-adapter");
  });
});
